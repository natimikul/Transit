                ...
                with col_c2:
                    if st.button(f"💾 Сохранить и распределить", type="primary", key=f"btn_created_{key_suffix}"):
                        try:
                            ...
                            # Строим словарь id → исходный статус (из df до редактирования)
                            orig_status_map = dict(zip(df['id'].astype(int), df['status']))

                            def _new_status(row):
                                inv_id = int(row.get('id') or 0)
                                orig = orig_status_map.get(inv_id, 'Создан')
                                if orig == 'Не обрабатывать':
                                    return 'Не обрабатывать'
                                perm_rb = row.get('perm_rb')
                                perm_kz = row.get('perm_kz')
                                # CheckboxColumn может вернуть True/False или 1/0
                                has_rb = bool(perm_rb) and perm_rb not in (0, '0', False, None, '')
                                has_kz = bool(perm_kz) and perm_kz not in (0, '0', False, None, '')
                                if has_rb or has_kz:
