...
def build_report(target_sheets, required_columns, filter_by_client=True, allowed_statuses=None, filter_by_invoice=True):
    frames = []
    ...
    if filter_by_invoice and 'счет_ввод' in globals() and счет_ввод:
        if 'Номер счета' in df_all.columns:
            # Разбиваем ввод пользователя по запятым и убираем лишние пробелы
            search_invoices = [inv.strip().lower() for inv in счет_ввод.split(',') if inv.strip()]
            if search_invoices:
                # Фильтруем таблицу, оставляя только совпадения по номерам
...
with c3:
    if st.button("🚚 Отгружено"):
        ...
        cols = ['№ заявки', '№ счета', 'Дата счета', 'Клиент', 'Плановая дата отгрузки', 'Дата отгрузки (факт)', 'Плановая дата прибытия', 'Статус']
        st.session_state.current_report = build_report(st.session_state.active_sheets, cols, filter_by_client=True, allowed_statuses=["Создан", "В сборке", "В пути", "Задержка поставки"])
...
with c4:
    if st.button("🏢 Прибытие"):
        ...
        st.session_state.current_report = build_report(st.session_state.active_sheets, cols, filter_by_client=True, allowed_statuses=["Прибыл на склад Алматы", "Готов к отгрузке клиенту"])
